...
public class ClientRpcWorker implements Runnable, IObserver {
    ...
    private Response handleRequest(Request request){
        ...
        if(request.type() == RequestType.GET_USER_BY_ID)
        {
            System.out.println("Get user by id ..." + request.type());
            try{
                return new Response.Builder().type(ResponseType.OK).data(server.getUserById((Integer) request.data())).build();
            } catch (ServicesException e) {
                return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
            }
        }
        if(request.type() == RequestType.GET_ALL_CONF)
        {
        ...
        if(request.type() == RequestType.GET_CONF_BY_ID)
        {
            System.out.println("Get conf by id ..." + request.type());
            try{
                return new Response.Builder().type(ResponseType.OK).data(server.getConfById((Integer) request.data())).build();
            } catch (ServicesException e) {
                return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
            }
        }
        if(request.type() == RequestType.GET_ALL_GAMES)
        {
            System.out.println("Get all games ..." + request.type());
            try{
                return new Response.Builder().type(ResponseType.OK).data(server.getAllGames()).build();
            } catch (ServicesException e) {
                return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
            }
        }
        if (request.type() == RequestType.ADD_CONFIGURATION)
        {

            System.out.println("Add configuration request ..." + request.type());
            Configuration configuration = (Configuration) request.data();
            try{
                Configuration addedConfiguration = server.addConfiguration(configuration);
                return new Response.Builder().type(ResponseType.OK).data(addedConfiguration).build();
            } catch (ServicesException e) {
                return new Response.Builder().type(ResponseType.ERROR).data(e.getMessage()).build();
            }
