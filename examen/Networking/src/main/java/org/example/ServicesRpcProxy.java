...
public class ServicesRpcProxy implements IServices {
    ...
    @Override
    public Collection<Game> getAllGames() throws ServicesException {
        Request request = new Request.Builder().type(RequestType.GET_ALL_GAMES).build();
        sendRequest(request);
        Response response = readResponse();
    ...
    public Configuration getConfById(Integer id) throws ServicesException {
        Request request = new Request.Builder().type(RequestType.GET_CONF_BY_ID).data(id).build();
        sendRequest(request);
        Response response = readResponse();

        if(response.type().equals(ResponseType.OK)){
            Configuration conf = (Configuration) response.data();
            return conf;
        }
    ...
    public User getUserById(Integer data) throws ServicesException {
        Request request = new Request.Builder().type(RequestType.GET_USER_BY_ID).data(data).build();
        sendRequest(request);
        Response response = readResponse();
    ...
    public Collection<Configuration> getAllConf() throws ServicesException {
        Request request = new Request.Builder().type(RequestType.GET_ALL_CONF).build();
        sendRequest(request);
        Response response = readResponse();

        if(response.type().equals(ResponseType.OK)){
          Collection<Configuration> confs = (Collection<Configuration>) response.data();
          return confs;
        }
    ...
    @Override
    public Configuration addConfiguration(Configuration configuration) throws ServicesException {
        Request request = new Request.Builder().type(RequestType.ADD_CONFIGURATION).data(configuration).build();
        sendRequest(request);
        Response response = readResponse();
